exports.handler = (event, context, callback) => {
    ...
    function server()
    {
      ...
      var order = process.env.order;
      
      // sort the priority in the sort
      var sort = process.env.sort;
      
      // site to search on stack exchange network
      var site = process.env.site;
      
      // tagged for which tag to use when searching for question
      var tagged = process.env.tagged;  
      
      targetUrl = targetUrl + '?order=' + order + '&sort=' + sort + '&site=' + site + '&tagged=' + tagged;
